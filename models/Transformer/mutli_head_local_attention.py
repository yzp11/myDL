...
class DotProductLocalAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None,window_size=50):
        ...
        mask = torch.zeros((batch,head, len, len),device=q.device)
        for i in range(len):
            mask[:,:, i, max(0, i - window_size):min(len, i + window_size)] = 1
...
class MutliHeadLocalAttention(nn.Module):

    def __init__(self, d_model, n_head, dropout=0.0):
        super(MutliHeadLocalAttention, self).__init__()
        self.n_head=n_head
        self.attention = DotProductLocalAttention(dropout=dropout)
